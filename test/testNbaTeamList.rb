...
class TestNbaTeamList < Minitest::Test
  ...
  def test_initialize
    tl = NbaTeamList.new
    numNbaTeams = 30 # Current number of NBA Teams
    # Test teamList()
    assert_equal numNbaTeams, tl.teamList.size, "NbaTeamList => Wrong # Teams"

    # Validate Header value
    assert_equal "NBA Teams", tl.header, "NbaTeamList => Wrong :header"


    # Test toString -> Validate Team List contents
    team_list_str = %q(BOS,Boston Celtics,Atlantic,Eastern
...
UTA,Utah Jazz,Northwest,Western
    ...
    assert_equal team_list_str, tl.toString, "NbaTeamList => String test failed"
  end
end
